...
export default {
	...
	paintChart(jsondata) {
        ...
        for(let item in jsondata['PCASPE']['xData']){
            if(jsondata['result'][item]['l']===-1){
                console.log(jsondata['result'][item])
            }
            wheeldata.push({
        ...
        class wheelRound{
            ...
            getProcess(_){
                for (let item in this.process){
                    if(this.process[item].indexOf(_)){
                        return item
                    }
                }
            }
            ...
            _process() {
                ...
                this._chartData = this._data.map(d => {
                    const datum = {
                        ...
                        date: d[field.date],
                        month: wheel.getProcess(d[field.date]),
                        low: d[field.low],
                        high: d[field.high],
                        avg: d[field.avg],
            ...
            _initDynamicParts() {
                ...
                this._highlight = this._g.append("g")
                    ...
                    .call(g => g.append("path")
                        ...
                        .attr("opacity", 0.3)
                        .attr("fill", "#999")
                        .attr("d", d => this._line(0, r.max - r.label)));
            ...
            _renderWheelContent() {
                ...
                vis.join("g")
                    ...
                    .call(g => g.append("path")
                        .attr("fill", 'purple')
                        .attr("opacity", 0.5)
                        .attr("d", d => this._line(r.outer+r.bubble*1.25-this._hb(d.humidity), r.outer+r.bubble*1.25)));
            ...
            _renderAxis() {
                this._g.append("g")
                    ...
                    .selectAll("g")
                    .data(this._y.ticks(1).slice(0, 5))
                    .join("g")
            ...
            _renderOverlay() {
                ...
                //         t.date.text(`Value: ${d.dateStr}`);
                //         t.high.text(`High: ${d.high}${this._tempUnit}`);
                //         t.low.text(`Low: ${d.low}${this._tempUnit}`);
                //         t.avg.text(`Avg: ${d.avg}${this._tempUnit}`);
                //         t.prec.text(`PCASPE.: ${d.precipitation}${this._precUnit}`);
                //         t.humidity.text(`PCAT2: ${d.humidity}`);
