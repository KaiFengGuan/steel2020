...
export default {
	...
	methods: {
		...
		paintChart(plData, startTime, endTime) {
			...
			var barScale = d3.map(barbin, array => d3.scalePow().domain([0, d3.max(d3.map(array, d => d.length))]).range([0, 35])),
				...
				x = new Map(
					Array.from(keys, key => [
						key,
						d3.scaleLinear(
							[barbin[keys.indexOf(key)][0].x0, barbin[keys.indexOf(key)].slice(-1)[0].x1],
			...
			class parallelLines{
				...
				_initControl(){
					...
					const plusG = this._mainG.append('g');
					const minusAttrs ={
					...
					minusG
						.selectAll('g').data(keys).join('g')
							...
							.on('click', function(e, d){
								...
								context.vNode.newBrushData[i][0] -= 10;
								context._updateArea()
							});
					plusG
						.selectAll('g').data(keys).join('g')
							...
							.on('click', function(e, d){
								const i = keys.indexOf(d);
								context.vNode.newBrushData[i][1] += 10;
								context._updateArea()
							});
				}
				_areaAttrs(){
					return {
						...
						datum: vm.newBrushData,
						opacity: 0.5,
						d: d3.area()
						.y((d, i) => y(newkeys[i]))
						.x0((d, i) => x.get(newkeys[i])(d[0]))
						.x1((d, i) => x.get(newkeys[i])(d[1])),
					}
				}
				_initArea(){
		...
		newpaintChart(plData, startTime, endTime) {
			let diagnosisSelected = new Map();
			plData = this.deepCopy(plData);
			this.plData = plData;
			var brushdata = plData.map(d => {
			...
			function diagnosisBrushSlider() {
				updateElement(svg.selectAll('.overlay'), overLayAttrs).raise();
				updateElement(svg.selectAll('.selection'), selectionAttrs).raise();
				// svg.selectAll('#parallel .parallelPath').raise()
				svg.selectAll('.handle--custom').attr('stroke', util.labelColor[0]);
				svg.selectAll('.handle--custom').raise();
				updateElement(svg.selectAll('.tick text'), tickTextAttrs);
			}
			this.svg !== undefined && this.svg.remove();
			this.svg = d3.select('#' + vm.menuId)
				.append('svg')
			...
			const brush = d3
				...
				.on('start brush end', this.diagnosisState ? diagnosisBrushed : basebrushed);
			...
			var selections = vm.diagnosisState ? this.newBrushSelection : this.brushSelection;
			svg
				.append('g')
			...
			function diagnosisBrushed({selection}, key){
				let selected = [];
