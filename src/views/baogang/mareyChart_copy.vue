...
export default {
    ...
    // console.log('filterdata:', d3.map(this.data, d=>d.stops.length).filter(d => d < 17));
    ...
    var data = alldata
		...
		var x = d3.scaleLinear()
			...
			.range([ mareyEntry , mareylength - 40])
		...
		var y = d3.scaleTime()
			...
			.range([mareyDistance + 10, height - margin.bottom])
    ...
    // mareylength- mareyEntry + 8 + labelwidth
        ...
        .data(stations)
				...
				.call(g => g.append("rect")
						...
						.attr("fill", (d,i) => d3.color(i < 6 ? stationcolor [0] : ( i> stations.length - 4 ? stationcolor [2] : stationcolor [1])))
						.attr("opacity", 0.6)
      ...
      // console.log('mergeresult:',mergeresult);
        ...
        // console.log('quality:',item, quality);
          ...
          mergeClickValue.push(i)
					vm.$emit("trainClick",{list: vm.trainSelectedList, color: vm.trainGroupStyle(mergeresult[i]["select"].slice(-1)[0]), 
		...
		function brushmove(event) {
			...
			y = d3.scaleTime()
				.domain(selectTime)
				.range([mareyDistance + 10, miniheight - miniMargin.bottom - miniMargin.top])
			line = d3.line()
				.x(d => x(d.station.distance))
				.y(d => y(new Date(d.time)))
			voronoi = Delaunay
    ...
    let miniBarsData = vm.isMerge ? filterdata : data
    // console.log(d3.map(miniBarsData, d => d.));
      ...
      .style("font-style", util.conditionMiniYAxisTextAttr.fontStyle)
		...
		setTrainColor(bool) {
			if (bool) {
				this.trainGroupStyle = d => d.flag !== 404 ? (d.flag === 0 ? this.labelColors[0] : this.labelColors[1]) : this.noflagColor
			} else {
          ...
          selectcolor = (d.flag !== 404 ? (d.flag === 0 ? this.labelColors[0] : this.labelColors[1]) : this.noflagColor)
				}else{
